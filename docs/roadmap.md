...
Status: S0 complete; S1 in progress. Signed receipt, queue processing, and transactional order effects are verified; status queries and the packaged demo remain pending. See the [processing checkpoint](s1-processing-checkpoint.md). Stages are acceptance gates, not automatic calendar deadlines. The original four-week sketch is an estimate for planning; development proceeds according to evidence and available integration access. See the [foundation checkpoint](bootstrap-checkpoint.md), [scaffold acceptance](s1-scaffold-checkpoint.md), [package acceptance](s1-package-checkpoint.md), and [current development setup](development.md).
...
| S1 — First vertical slice | Rails API/MySQL setup, four package boundaries, signed order-create fixture, HMAC ingress, canonical-event constraint, job, order projection/effect, and event query | Clean-clone demo; ten duplicate deliveries yield one canonical event and one database domain effect; MySQL-backed CI; evidence for a `v0.0.1` release | In progress; queued order-create effects verified; tenant-scoped status queries next |
| S2 — Failure and recovery | Effect transaction proof, bounded retry, dead-letter, recovery, concurrency/crash tests, and explicit out-of-order transitions | 100 sequential and 100 concurrent duplicates produce one effect; acceptance/enqueue-gap and post-effect-crash recovery pass; add updated/cancelled topics only after S1 | Initial claim/retry/recovery implemented; full failure evidence pending |
...
Implement tenant-scoped status queries (#10), following the [verified processing checkpoint](s1-processing-checkpoint.md), then package the ten-delivery/one-effect demo (#11). The initial claim/retry and graceful restart evidence does not complete the full failure suite or overall goal.
