...
| S1 — First vertical slice | Rails API/MySQL setup, four package boundaries, signed order-create fixture, HMAC ingress, canonical-event constraint, job, order projection/effect, and event query | Clean-clone demo; ten duplicate deliveries yield one canonical event and one database domain effect; MySQL-backed CI; evidence for a `v0.0.1` release | In progress; scaffold, package contracts, boundary probes and CI implemented; signed fixture/normalization next |
