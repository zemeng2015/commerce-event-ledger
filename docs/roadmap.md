...
Status: S0 foundation. There is no Rails application or measured runtime behavior yet. Stages are acceptance gates, not automatic calendar deadlines. The original four-week sketch is an estimate for planning; development proceeds according to evidence and available integration access.
...
| --- | --- | --- | --- |
| S0 — Foundation | Repository, MIT license, contributor and security guidance, charter, architecture, goal, and a focused first backlog | Documentation reviewed; repository prepared for development; planned checks distinguished from runtime checks | Current stage; repository foundation is being established |
