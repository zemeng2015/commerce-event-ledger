...
Implemented for issues #7 and #8. See the [verified receipt checkpoint](s1-receipt-checkpoint.md) for native MySQL, Docker, and repeated race evidence. See the subsequent [processing checkpoint](s1-processing-checkpoint.md) for queue and initial recovery evidence.
...
The receipt adapter rejects an existing enclosing transaction, ensuring it cannot return before the actual commit. Insert failures roll back shop registration and the event together. MySQL deadlocks permit at most five whole-transaction attempts, with 5/10/15/20 ms delays between attempts; exhaustion returns a sanitized `503`. Existing-tenant requests do not acquire a tenant-wide write lock. The concurrency test retains an existing shop so contenders compete on the canonical event key itself. A receipt means durable pending work only. Solid Queue dispatch now follows commit; an enqueue failure retains the acknowledged event for scheduled recovery. Transactional effects are asynchronous, so an HTTP acknowledgment does not promise completed processing. GraphQL inspection remains pending. Receipt tests alone do not prove one effect.
