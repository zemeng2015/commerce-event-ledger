...
Implemented for issues #7 and #8. See the [verified receipt checkpoint](s1-receipt-checkpoint.md) for native MySQL, Docker, and repeated race evidence. Processing and recovery remain pending.
...
The receipt adapter rejects an existing enclosing transaction, ensuring it cannot return before the actual commit. Insert failures roll back shop registration and the event together. MySQL deadlocks permit at most five whole-transaction attempts, with 5/10/15/20 ms delays between attempts; exhaustion returns a sanitized `503`. Existing-tenant requests do not acquire a tenant-wide write lock. The concurrency test retains an existing shop so contenders compete on the canonical event key itself. A receipt means durable pending work only. Solid Queue dispatch, effects, recovery, and GraphQL inspection are the next dependencies and remain unimplemented. Do not claim a processed order or one domain effect from receipt tests.
