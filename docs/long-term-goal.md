...
Status: **active — S0 complete; S1 queued order-create processing verified; status queries and packaged demo pending**. These increments do not complete this goal. Status queries, integration, full failure recovery evidence, and release work remain outstanding. See the [processing checkpoint](s1-processing-checkpoint.md). See the [receipt acceptance](s1-receipt-checkpoint.md), earlier [fixture acceptance](s1-fixture-checkpoint.md), and [development setup](development.md).
...
| S1 / `v0.0.1` | Signed order-create vertical slice: ten duplicates, one canonical event, one effect, MySQL-backed checks | In progress — receipt, queue processing and transactional order effects verified; queries and packaged demo pending |
...
Next action: implement [issue #10: tenant-scoped GraphQL event status](https://github.com/zemeng2015/commerce-event-ledger/issues/10), then package the repeatable demo in #11. The [processing checkpoint](s1-processing-checkpoint.md) records 128 passing tests in each environment, one-effect HTTP evidence, and twenty graceful worker restarts. These restarts do not satisfy the full forced-crash suite. Continue only `orders/create` until S1 acceptance is complete.
